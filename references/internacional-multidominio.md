...
- **`.net` e `.org` recebem *targeting* explícito no Google Search Console** quando o público for regional, já que o TLD não carrega geo. `.org` segue a convenção institucional/informacional — o Google não força, mas usuários e linkadores esperam isso, e a casa honra.
...
- **`url` do `Organization` aponta pro domínio-âncora (`.net`)** como URL canônica da entidade, mesmo servido a partir do `.org`/`.me`/`.com.br` — isso reforça o `.net` como face comercial sem canonicalizar as páginas.
...
- [ ] **Targeting geográfico no GSC** configurado pros gTLDs quando o público é regional (§3).
